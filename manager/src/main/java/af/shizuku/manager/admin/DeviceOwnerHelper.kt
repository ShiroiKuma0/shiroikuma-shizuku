...
object DeviceOwnerHelper {
    ...
    fun clearDeviceOwner(context: Context, onCleared: () -> Unit = {}) {
        ...
        if (!dpm.isDeviceOwnerApp(context.packageName)) {
            showClearFailure(
                context,
                "This app is not the Device Owner, so there is nothing to clear.\n\n" +
        ...
        try {
            ...
            Timber.e(e, "clearDeviceOwnerApp failed")
            showClearFailure(
                context,
                "${e.javaClass.simpleName}: ${e.message ?: "no message"}\n\n" +
                    "Package: ${context.packageName}\n\n" +
                    "The device is still Device Owner. Do NOT uninstall the app in this state — " +
                    "removing it while it holds Device Owner leaves a factory reset as the only exit."
            )
            return
        }

        // clearDeviceOwnerApp() is documented as best-effort, so confirm rather than assume.
        if (dpm.isDeviceOwnerApp(context.packageName)) {
            showClearFailure(
                context,
                "The call returned without error, but this app is still reported as Device Owner.\n\n" +
                    "Package: ${context.packageName}\n\n" +
